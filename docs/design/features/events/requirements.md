...
### 2.4 用户事件阅读状态表 (user_event_reads)

为支持事件中心的"未读/已读"功能，新增用户事件阅读状态表：

```sql
-- 用户事件阅读状态表
CREATE TABLE user_event_reads (
  id BIGSERIAL PRIMARY KEY,                                    -- 记录ID
  user_id BIGINT NOT NULL,                                     -- 用户ID(虚拟外键 -> users.id)
  event_id BIGINT NOT NULL,                                    -- 事件ID(虚拟外键 -> events.id)
...
CREATE INDEX idx_user_event_reads_user_event ON user_event_reads(user_id, event_id);

COMMENT ON TABLE user_event_reads IS '用户事件阅读状态表:记录用户标记已读的事件';
COMMENT ON COLUMN user_event_reads.read_at IS '用户标记事件为已读的时间';
```

**设计说明**：

1. **为什么不在events表中存储阅读状态**：
   - events表是全局事件表，不应包含用户相关数据
   - 使用独立表可避免events表膨胀
   - 支持多用户独立阅读状态

2. **查询优化**：
   - 查询用户未读事件：`SELECT * FROM events WHERE id NOT IN (SELECT event_id FROM user_event_reads WHERE user_id = ?)`
   - 或使用LEFT JOIN：`SELECT e.* FROM events e LEFT JOIN user_event_reads r ON e.id = r.event_id AND r.user_id = ? WHERE r.id IS NULL`
